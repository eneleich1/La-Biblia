...
function SectionTitle({ children }: { children: string }) {
  return (
    <div className="flex items-center gap-3">
      <span className="h-px w-7 shrink-0 bg-[var(--accent)]" aria-hidden />
      <h2 className="font-serif-display text-lg font-semibold text-[var(--text)] sm:text-[1.05rem]">
        {children}
      </h2>
    </div>
  );
...
function HeroIllustrationFallback() {
  return (
    <div className="relative h-full min-h-[7rem] w-full overflow-hidden" aria-hidden>
      ...
      <img
        src="/apologetica/icons/hero-illustration.svg"
        alt=""
        className="absolute inset-y-0 right-0 h-full w-full object-cover object-center opacity-90"
      />
      <div className="absolute inset-0 bg-gradient-to-r from-[#f7f1e8] via-[#f7f1e8]/30 to-transparent" />
    </div>
  );
}
...
function HeroFeatureCard() {
  ...
  return (
    <div className="min-w-0 w-full flex-1 overflow-hidden rounded-md border border-[var(--border)] bg-[#f7f1e8] shadow-[var(--shadow-card)]">
      <div className="grid min-h-[7.5rem] lg:grid-cols-[minmax(0,1fr)_14.5rem_minmax(8.5rem,10.25rem)] lg:items-stretch">
        ...
        <ul className="flex flex-col justify-center gap-2.5 border-t border-[var(--border)]/70 bg-[#f7f1e8]/80 px-4 py-3 lg:border-l lg:border-t-0">
...
function GuideIcon({ guide }: { guide: ApologeticaGuide }) {
  const sizeClass = "h-[4rem] w-[4rem] shrink-0 sm:h-[4.35rem] sm:w-[4.35rem]";
  ...
  return (
    ...
    >
      <Icon className="h-8 w-8 text-[var(--accent)] sm:h-9 sm:w-9" strokeWidth={1.5} aria-hidden />
    </div>
  );
}
...
function GuideCard({ guide }: { guide: ApologeticaGuide }) {
  return (
    <Link
      ...
      className="group relative flex min-h-[7.15rem] gap-4 rounded-md border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-card)] transition hover:border-[var(--accent)]/35 hover:shadow-[var(--shadow-card-hover)] no-underline"
    >
      <Bookmark
        className="absolute right-4 top-3.5 h-3.5 w-3.5 text-[var(--accent)]"
        strokeWidth={1.75}
        aria-hidden
      />
      <GuideIcon guide={guide} />
      <div className="min-w-0 flex-1 pr-5">
        <h3 className="font-serif-display text-[15px] font-semibold leading-snug text-[var(--text)] sm:text-base">
          {guide.title}
        </h3>
        <p className="mt-2 max-w-[28rem] text-xs leading-relaxed text-[var(--text-muted)]">
          {guide.description}
        </p>
        <span className="mt-3 inline-flex items-center gap-0.5 text-xs font-semibold text-[var(--accent)] transition group-hover:gap-1">
          Abrir guía
...
function ResourceCard({ resource }: { resource: ApologeticaResource }) {
  ...
  return (
    <article className="flex h-full min-h-[6.8rem] flex-col rounded-md border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-card)]">
      <div className="flex items-start gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f5efe6] text-[var(--accent)]">
          <Icon className="h-4 w-4" strokeWidth={1.65} aria-hidden />
        </span>
        <div className="min-w-0">
          <h3 className="font-serif-display text-sm font-semibold leading-snug text-[var(--text)]">
            {resource.title}
          </h3>
      ...
      <span className="mt-3 inline-flex w-fit items-center justify-center gap-1.5 rounded-md border border-[var(--border)] bg-[var(--background-soft)] px-3 py-1.5 text-[10px] font-medium text-[var(--text-muted)]">
...
export function ApologeticaIndex() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1040px] space-y-5 pb-2 lg:space-y-6">
      <nav className="text-xs text-[var(--text-muted)]" aria-label="Migas de pan">
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:gap-10">
        <header className="w-full shrink-0 space-y-2 lg:w-[min(100%,15.5rem)] lg:pt-2">
          <h1 className="font-serif-display text-4xl font-semibold tracking-normal text-[var(--text)] sm:text-[2.45rem]">
            Apologética
          </h1>
          <p className="max-w-[14.5rem] text-sm leading-relaxed text-[var(--text-muted)] sm:text-[14px]">
            {apologeticaPageDescription}
          </p>
        </header>
        <HeroFeatureCard />
      ...
      {activeTab === "guias" ? (
        <>
          <section className="space-y-4" aria-labelledby="guias-apologetica">
          ...
          <section className="space-y-4" aria-labelledby="mas-recursos-apologetica">
